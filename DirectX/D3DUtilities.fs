...
module D3DUtilities =
    ...
    [<AllowNullLiteral>] 
    type Material (name:string, matCBIndex:int, diffuseSrvHeapIndex:int, diffuseAlbedo:Vector4, fresnelR0:Vector3, roughness:float32, hasTexture:bool) =
        ...
        new (name, constants:MaterialConstants) = Material (name,  0, 0, constants.DiffuseAlbedo,constants.FresnelR0 ,  constants.Roughness, false)
    ...
    type Texture (name:string, filename:string, resource:Resource) =
        let mutable name=name
