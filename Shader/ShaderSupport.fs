...
module ShaderSupport = 
    ...
    type ShaderDescription =
        ...
        val Entry:string
        val Mode:string
        new (klass, application, directory, file, entry, mode) =
            {Klass=klass; Application=application; Directory=directory; File=file; Entry=entry; Mode=mode}

        new () = {Klass=ShaderClass.NotSet; Application=""; Directory=""; File=""; Entry=""; Mode=""}

        member self.IsEmpty() = 
            self.Klass=ShaderClass.NotSet
