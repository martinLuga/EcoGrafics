...
module MYUtils = 
    ...
    type ObjectControlblock (StartVertices:int, StartIndices:int, EndVertices:int, EndIndices:int, IndexCount:int) =
    ...
    // ----------------------------------------------------------------------------------------------------
    [<AllowNullLiteralAttribute>]
    type GeometryCache(device:Device) =
        let mutable device=device
        ...
        member this.Append(geometryName, vertices, indices:int[])=
            let mutable ocb = 
                ObjectControlblock(
